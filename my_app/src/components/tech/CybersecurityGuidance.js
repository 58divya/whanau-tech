...
const CybersecurityGuidance = ({ selectedLanguage }) => {
  const t = translations[selectedLanguage]?.cybersecurityPage || translations.en.cybersecurityPage;

  return (
    <div className="tech-consultation-page min-vh-100">
      <header className="hero-section py-5 text-white text-center">
        <div className="container">
          <h1 className="display-4 fw-bold mb-4 animate__animated animate__fadeIn">{t.title}</h1>
          <p className="lead mb-4">{t.subtitle}</p>
          <Link to="/advisor/1" className="btn btn-primary btn-lg" aria-label={t.contactButton}>
            {t.contactButton}
          </Link>
        </div>
      </header>
      ...
      <main className="container py-5">
        ...
        <section className="why-section mb-5 bg-white p-4 rounded shadow">
          ...
          <p className="fs-5 text-muted">{t.whyText}</p>
        </section>
                ...
                <div key={index} className={`carousel-item ${index === 0 ? 'active' : ''}`}>
                  <blockquote className="bg-white p-4 rounded shadow text-center">
                    <p className="fs-5 fst-italic text-muted">
                      "{testimonial.quote}" <br />– {testimonial.author}
                    </p>
                  </blockquote>
                </div>
              ))}
            </div>
            <button className="carousel-control-prev" type="button" data-bs-target="#cybersecurityCarousel" data-bs-slide="prev">
              <span className="carousel-control-prev-icon" aria-hidden="true"></span>
            </button>
            <button className="carousel-control-next" type="button" data-bs-target="#cybersecurityCarousel" data-bs-slide="next">
